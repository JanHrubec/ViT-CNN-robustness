...
IN_MEAN_FILL_255 = [123, 116, 103]
IN_MEAN_FILL_01 = [123 / 255.0, 116 / 255.0, 103 / 255.0]
...
def pil_to_tensor01(pil_image: Image.Image) -> torch.Tensor:
    return F.pil_to_tensor(pil_image.convert("RGB")).float() / 255.0


def apply_rotation_pil(pil_image: Image.Image, deg: float) -> Image.Image:
    return F.rotate(
        pil_image,
        angle=deg,
        interpolation=InterpolationMode.BILINEAR,
        fill=IN_MEAN_FILL_255,
    )


def apply_translation_x_pil(pil_image: Image.Image, px: int) -> Image.Image:
    x = pil_to_tensor01(pil_image)
    x2 = F.affine(
        x,
        angle=0.0,
        ...
        shear=[0.0, 0.0],
        interpolation=InterpolationMode.BILINEAR,
        fill=IN_MEAN_FILL_01,
    )
    return tensor01_to_pil(x2)


def apply_translation_y_pil(pil_image: Image.Image, px: int) -> Image.Image:
    x = pil_to_tensor01(pil_image)
    x2 = F.affine(
        x,
        angle=0.0,
...
def apply_gaussian_noise_pil(pil_image: Image.Image, sigma: float, rng: np.random.Generator) -> Image.Image:
    if sigma <= 0:
        return pil_image
...
def apply_corruption_spec_pil(spec: CorruptionSpec, pil_image: Image.Image, seed: int) -> Image.Image:
    ...
    if spec.family == "rotation":
        return apply_rotation_pil(pil_image, spec.severity)
    if spec.family == "translation_x":
        return apply_translation_x_pil(pil_image, int(spec.severity))
    if spec.family == "translation_y":
        return apply_translation_y_pil(pil_image, int(spec.severity))
    if spec.family == "gaussian_noise":
        return apply_gaussian_noise_pil(pil_image, float(spec.severity), rng)
...
def make_corruption_transform(spec: CorruptionSpec, preprocess: Callable, seed: int) -> Callable:
    ...
    def transform(img) -> torch.Tensor:
        if spec.family == "gaussian_noise" and gaussian_rng is not None:
            corrupted = apply_gaussian_noise_pil(img, float(spec.severity), gaussian_rng)
        else:
