...
def compute_prediction_stability(per_sample_rows: Iterable[dict]) -> list[dict]:
    ...
    for row in per_sample_rows:
        ...
        if model is None or idx is None or pred is None:
            continue
        clean_pred[(str(model), int(repeat), int(idx))] = int(pred)

    grouped: dict[tuple[str, int, str, float], list[int]] = defaultdict(list)
    ...
    for row in per_sample_rows:
        ...
        if model is None or idx is None or pred is None or family is None or severity is None:
            continue
        key = (str(model), int(repeat), str(family), float(severity))
        clean = clean_pred.get((str(model), int(repeat), int(idx)))
        if clean is None:
            continue
        grouped[key].append(1 if int(pred) == clean else 0)

    out: list[dict] = []
    for (model, repeat, family, severity), hits in sorted(grouped.items()):
        if not hits:
            continue
        out.append(
            {
